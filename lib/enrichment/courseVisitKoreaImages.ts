...
import {
  collectVisitKoreaMetaImagePaths,
  type VisitKoreaMetaImageFields,
} from "@/lib/visitKoreaMetaImages";
...
type VisitKoreaMetaEntry = VisitKoreaMetaImageFields & {
  courseId?: string;
  contentId?: string;
};
...
function collectImagesFromEntry(entry: VisitKoreaMetaEntry): string[] {
  return collectVisitKoreaMetaImagePaths(entry);
}
...
function buildAppliedIndex(): Map<string, CourseVisitKoreaImageSet> {
  ...
  for (const item of appliedStore.items ?? []) {
    const images = item.images.filter(isAllowedVisitKoreaImageUrl);
...
export function resolveCourseVisitKoreaImages(
  ...
  const fromEnrichment = enrichment?.visitKoreaImages?.filter(
    isAllowedVisitKoreaImageUrl,
  );
