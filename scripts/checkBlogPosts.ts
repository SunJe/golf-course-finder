...
function main(): void {
  ...
  if (BLOG_POSTS.length !== 32) {
    fail(`Expected 32 posts, got ${BLOG_POSTS.length}`);
  }
  ...
  if (CATEGORY_COUNTS["course-guide"] !== 14) {
    fail(`Expected 14 course-guide posts, got ${CATEGORY_COUNTS["course-guide"]}`);
  }
